...
if( isset($_POST['submit_user_forgot']) ) {
    $email = $_POST['email'];
    if( $email == "" ) {
        ...
        $checkSQL = mysqli_query($db, "SELECT * FROM `js_seeker` WHERE `Seeker_Email` = '".$email."' LIMIT 0, 1");
        if( $checkSQL ) {
            if( mysqli_num_rows($checkSQL) > 0 ) {
                $seeker    = mysqli_fetch_assoc($checkSQL);
                $fullname  = $seeker['Seeker_Name'];
                $chars     = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
                $password  = substr( str_shuffle( $chars ), 0, 7 );
                $updateSQL = mysqli_query($db, "UPDATE `js_seeker` SET `Seeker_Password` = '".sha1($password)."' WHERE `Seeker_Email` = '".$email."'");
...
$page_name = 'Seeker Forgot Password';
...
<body>
    ...
    <div class="content-area">
        <div class="banner-area">
            <img src="http://localhost/jobseeker/assets/img/login_banner.jpg" alt="Seeker Forgot Password" />
        </div>
        <div class="container">
            <div class="row">
                <h1 class="text-center">Seeker Forgot Password</h1>
                ...
                <form action="" method="post">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            <input type="submit" name="submit_user_forgot" id="submit_user_forgot" value="Reset" class="btn btn-success" />
                        </div>
                    </div>
                </form>
                <div class="col-sm-12">
